...
static struct task_struct *rr_curr = NULL;
static LIST_HEAD(rr_queue);
...
static void enqueue_task_fair(struct rq *rq, struct task_struct *p, int flags) {
    list_add_tail(&p->rr_list, &rr_queue);
    rr_curr = NULL;  // trigger re-selection
    rq->nr_running++;
}
...
struct task_struct *pick_next_task_fair(struct rq *rq, struct task_struct *prev, struct rq_flags *rf)
{
    if (!list_empty(&rr_queue)) {
        rr_curr = list_first_entry(&rr_queue, struct task_struct, rr_list);
        list_move_tail(&rr_curr->rr_list, &rr_queue);
        return rr_curr;
    }
    return NULL;
}
...
{
    curr->se.exec_start++;
    if (curr->se.exec_start >= RR_TIMESLICE)
        resched_curr(rq);
}
